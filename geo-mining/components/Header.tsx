...
const navigation: NavItem[] = [
  ...
  { 
    ...
    href: '/subsidiaries',
    children: [
      { name: 'StoneCrest Resource Limited', href: '/subsidiaries#stonecrest-resource' },
      { name: 'StoneCrest Mining Corporation', href: '/subsidiaries#stonecrest-mining' },
    ]
  },
  { 
    ...
    children: [
      ...
      { name: 'Community Development & CDA', href: '/sustainability#community' },
      { name: 'Health & Safety', href: '/sustainability#safety' },
      { name: 'Rehabilitation & Closure', href: '/sustainability#rehabilitation' },
    ]
  },
  { 
    ...
    children: [
      { name: 'Geological & Environmental Consultancy', href: '/projects-services#consultancy' },
      ...
      <div className="bg-[#2d5a27] text-white py-2 px-4 md:px-8">
        <div className="max-w-7xl mx-auto flex justify-between items-center text-[10px] md:text-xs font-bold uppercase tracking-widest">
          ...
          <div className="hidden md:flex gap-6 text-slate-100">
            <a href="#" className="hover:text-amber-400 transition-colors">Client Portal</a>
          ...
          <div className="flex justify-between items-center h-24 md:h-32">
            ...
            <Link to="/" className="flex-shrink-0 flex items-center gap-4 group">
              ...
              <div className="flex flex-col">
                <span className="text-[#2d5a27] font-black text-xl md:text-2xl leading-none tracking-tight">GeoMining</span>
                ...
                <div key={item.name} className="relative group h-full flex items-center">
                  <Link
                    ...
                    className={`px-4 py-2 font-bold text-[10px] uppercase tracking-wider transition-all flex items-center gap-1.5 h-full ${
                      isActive(item.href) ? 'text-[#2d5a27] border-b-4 border-[#2d5a27]' : 'text-slate-600 hover:text-[#2d5a27]'
                    }`}
                  >
                    {item.name}
              ...
              <div key={item.name} className="border-b border-slate-50 last:border-0">
                <div className="flex justify-between items-center py-1">
                  ...
                  {item.children && (
                    ...
                    >
                      {openSubmenu === item.name ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                    </button>
                  )}
                </div>
